...
nonOverlapping xs =
  foldl'
    ( \intervals (this, next) ->
        let open x = x {_intervalEndpoint_type = Open}
...
wordCharacterRanges t =
  let (lastIndex, lastPrevBreak, lastRanges) =
        T.foldl
          ( \(!index, !prevBreak, ranges) c ->
...
appWidget referenceDyn = do
  ...
  wordClickedRaw :: Event t (IntervalEndpoint (VerseReference, Int), CharacterIndex Int) <-
    mapMaybe
      ...
      <$> selectionStart
  rec (verses, tags) <- watchVerses $ (defaultTranslation,) <$> range
      let verseRanges = fmap versesToRanges <$> verses
          tagRanges = tagsToRanges <$> tags
      let wordClicked :: Event t (VerseReference, Int) =
            attachWithMaybe
      ...
      (cursorMode, selectedRanges, currentTagName) <- divClass "columns" $ do
        ...
        divClass "column" $ do
          cursorMode_ :: Dynamic t CursorMode <- divClass "tabs is-toggle is-fullwidth is-small" $ el "ul"
            $ toggleTabList CursorMode_Select [CursorMode_Select, CursorMode_Highlight]
            $ text . \case
          ...
          rec selectedRanges_ <-
              ...
              ((), tagDeleted) <- runEventWriterT $ dyn_ $ ffor selectedRanges_' $ \case
                ...
                Just selRanges -> do
                  ...
                  divClass "columns"
                    $ void
                    $ listWithKey (Map.fromDistinctAscList . IntervalMap.toAscList <$> selRanges)
                    $ \tagRange tagNames ->
                      void $ listWithKey (Map.fromSet (,()) <$> tagNames) $ \tagName _ -> divClass "column is-full" $ do
                        ...
                        rec let newValueFromServer =
                                  mapMaybe
                                    ( \(localState, (serverText, serverTime)) ->
                            ...
                            notesText <-
                              holdUniqDyn <=< fmap value $ textAreaElement $
                                def
      ...
      let captureRange :: Maybe (VerseReference, Int) -> (VerseReference, Int) -> Maybe ((VerseReference, Int), (VerseReference, Int))
          captureRange firstWord lastWord = firstWord <&> \fstWord ->
            if lastWord >= fstWord
      ...
      let highlightFinishedWithTagName =
            mapMaybe
              (\(a, b) -> liftA2 (,) a (Just b))
      ...
      range <-
        foldDyn ($) (IntervalCO (VerseReferenceT (BookId 1) 1 1) (VerseReferenceT (BookId 1) 3 9999)) $
          leftmost
  ...
  where
    ...
    referenceToInterval (VerseReferenceT (BookId b) c _) =
      IntervalCO
    ...
    versesWidget verseRanges'' tagRanges selected = mdo
      ...
      dyn_ $ ffor verseRanges' $ \case
        ...
        Just verseRanges -> do
          let wordMap = verseWordMap <$> verseRanges
          ...
          void $ listWithKey domSegments $ \k _ -> do
            ...
            case startingEndpoint of
              ...
              _ -> blank
            let wordsDyn = T.intercalate " " . map snd . IntervalMap.toAscList . (`IntervalMap.intersecting` k) <$> wordMap
                relevantTags = (`IntervalMap.intersecting` k) <$> tagRanges
                isSelected = maybe False (not . null . (`IntervalMap.intersecting` k)) <$> selected
            colorSeed :: Dynamic t (Maybe Int) <-
              holdUniqDyn $
                fmap (hash . T.intercalate "," . toList) . nonEmpty . Set.toAscList . fold
                  <$> relevantTags
            color <- holdUniqDyn $ ffor2 isSelected colorSeed $ \sel colorSeed' ->
              if sel
                then Just ("pink", "black")
                else ffor colorSeed' $ \seed ->
                  let RGB r g b = toSRGB24 $ fst $ runIdentity $ flip runRandT (mkStdGen seed) $ randomColor HueRandom LumBright
...
toggleTabList v0 options showOption = do
  rec clickOptions <- for options $ \o -> do
        (e, ()) <-
          elDynAttr'
            "li"
...
watchTranslations =
  (fmap . fmap) (fmap (fmap getFirst . snd) . getOption . _view_translations)
    $ watchViewSelector
    $ pure
    $ mempty {_viewSelector_translations = Option $ Just 1}
...
watchVerses rng = do
  result <- watchViewSelector $ ffor rng $ \(translationId, interval) ->
    mempty
...
watchTagNotes kDyn = do
  resultDyn <- watchViewSelector $ ffor kDyn $ \k ->
    mempty
