...
viewSelectorHandler runTransaction vs =
  if vs == mempty
    then pure mempty
      ...
      let tagNotes = flip MMap.mapMaybeWithKey tagNotes' $ \k v -> do
...
getTagsInInterval translationId interval = fmap (MMap.fromAscListWith (<>) . map (second $ Set.singleton . pairToClosedInterval))
  ...
  $ do
    ...
    pure
      ( _tagName tag,
        ( _taggedrangeStart taggedRange,
          _taggedrangebywordStart taggedRangeByWord,
          _taggedrangeEnd taggedRange,
          _taggedrangebywordEnd taggedRangeByWord
        )
      )
  where
...
guardExactTagRangeMatches ::
  ( SqlIn (QGenExpr QValueContext Pg.Postgres s) (Columnar f1 Text),
    ...
    SqlValable (Columnar f1 Text),
    SqlValable (Columnar f2 Int),
    SqlValable (VerseReferenceT f3),
    HaskellLiteralForQExpr (Columnar f1 Text) ~ Text,
